...
DEFAULT_NPM_REGISTRY="${OPENCLAW_NPM_REGISTRY:-https://registry.npmmirror.com}"
DEFAULT_GIT_REPO_URL="${OPENCLAW_GIT_REPO_URL:-https://github.com/Jakecoin/longxia.git}"
DEFAULT_NODE_MANUAL_URL="${OPENCLAW_NODE_MANUAL_URL:-https://npmmirror.com/mirrors/node/}"
DEFAULT_HOMEBREW_INSTALL_URL="${OPENCLAW_HOMEBREW_INSTALL_URL:-https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh}"
DEFAULT_NODESOURCE_DEB_SETUP_URL="${OPENCLAW_NODESOURCE_DEB_SETUP_URL:-https://deb.nodesource.com/setup_22.x}"
DEFAULT_NODESOURCE_RPM_SETUP_URL="${OPENCLAW_NODESOURCE_RPM_SETUP_URL:-https://rpm.nodesource.com/setup_22.x}"
...
bootstrap_gum_temp() {
    ...
    base="${OPENCLAW_GUM_BASE_URL:-https://github.com/charmbracelet/gum/releases/download/v${GUM_VERSION}}"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, run: powershell -ExecutionPolicy Bypass -File .\\scripts\\install.ps1"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="./docs/help/faq.md"
    if [[ -n "$GUM" ]]; then
        local content
...
run_npm_global_install() {
    ...
    local -a cmd
    cmd=(env "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" "npm_config_registry=$DEFAULT_NPM_REGISTRY" npm --loglevel "$NPM_LOGLEVEL")
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/longxia)
...
Environment variables:
  ...
  OPENCLAW_GIT_REPO_URL=<self-hosted mirror or approved git remote>
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  bash scripts/install.sh
  bash scripts/install.sh --no-onboard
  OPENCLAW_GIT_REPO_URL=<mirror> bash scripts/install.sh --install-method git --no-onboard
EOF
}
...
install_homebrew() {
    if [[ "$OS" == "macos" ]]; then
        if ! command -v brew &> /dev/null; then
            ...
            run_quiet_step "Installing Homebrew" run_remote_bash "$DEFAULT_HOMEBREW_INSTALL_URL"
...
install_node() {
    if [[ "$OS" == "macos" ]]; then
        ...
        if command -v apt-get &> /dev/null; then
            local tmp
            tmp="$(mktempfile)"
            download_file "$DEFAULT_NODESOURCE_DEB_SETUP_URL" "$tmp"
            if is_root; then
            ...
            local tmp
            tmp="$(mktempfile)"
            download_file "$DEFAULT_NODESOURCE_RPM_SETUP_URL" "$tmp"
            if is_root; then
                run_quiet_step "Configuring NodeSource repository" bash "$tmp"
            ...
            echo "Please install Node.js 22+ manually from a reachable mirror: ${DEFAULT_NODE_MANUAL_URL}"
            exit 1
        fi
...
ensure_pnpm() {
    ...
    fix_npm_permissions
    run_quiet_step "Installing pnpm" env "npm_config_registry=$DEFAULT_NPM_REGISTRY" npm install -g pnpm@10
    refresh_shell_command_cache
    if detect_pnpm_cmd && pnpm_cmd_is_ready; then
        ui_success "pnpm ready ($(pnpm_cmd_pretty))"
        return 0
...
EOF
    ...
    ui_info "Install pnpm globally (npm_config_registry=${DEFAULT_NPM_REGISTRY} npm install -g pnpm@10) and retry"
    return 1
}
...
install_openclaw_from_git() {
    local repo_dir="$1"
    local repo_url="$DEFAULT_GIT_REPO_URL"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing OpenClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing OpenClaw from remote checkout (${repo_url})"
    fi
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "bash scripts/install.sh --install-method npm"
