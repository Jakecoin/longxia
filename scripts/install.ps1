...
# Or: powershell -ExecutionPolicy Bypass -File .\scripts\install.ps1 -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\longxia",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
$NC = "`e[0m"                     # No Color
$script:DefaultNpmRegistry = if ($env:OPENCLAW_NPM_REGISTRY) { $env:OPENCLAW_NPM_REGISTRY } else { "https://registry.npmmirror.com" }
$script:DefaultGitRepoUrl = if ($env:OPENCLAW_GIT_REPO_URL) { $env:OPENCLAW_GIT_REPO_URL } else { "https://github.com/Jakecoin/longxia.git" }
$script:DefaultNodeMirrorUrl = if ($env:OPENCLAW_NODE_MANUAL_URL) { $env:OPENCLAW_NODE_MANUAL_URL } else { "https://npmmirror.com/mirrors/node/" }
...
function Install-Node {
    ...
    Write-Host "Please install Node.js 22+ manually from a reachable mirror: $script:DefaultNodeMirrorUrl" -Level info
    return $false
}
...
function Install-Git {
    ...
    Write-Host "Please install Git for Windows manually, then rerun the installer" -Level error
    return $false
}
...
function Install-OpenClawNpm {
    ...
    try {
        ...
        npm install -g openclaw@$Version --registry $script:DefaultNpmRegistry --no-fund --no-audit 2>&1
...
function Install-OpenClawGit {
    ...
    if (!(Test-Path $RepoDir)) {
        ...
        git clone $script:DefaultGitRepoUrl $RepoDir 2>&1
    } elseif ($Update) {
    ...
    if (!(Get-Command pnpm -ErrorAction SilentlyContinue)) {
        ...
        npm install -g pnpm --registry $script:DefaultNpmRegistry 2>&1
    }
    
    # Install dependencies
    Write-Host "  Installing dependencies..." -Level info
    pnpm install --dir $RepoDir --registry $script:DefaultNpmRegistry 2>&1
