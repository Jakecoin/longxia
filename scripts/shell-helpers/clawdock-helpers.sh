...
# Inspired by a local Docker helper workflow for Longxia
#
# Installation:
#   mkdir -p ~/.clawdock && cp scripts/shell-helpers/clawdock-helpers.sh ~/.clawdock/clawdock-helpers.sh
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    echo "Clone it first:"
    echo ""
    echo "  git clone <your-approved-longxia-remote> ~/longxia"
    echo "  cd ~/longxia && ./docker-setup.sh"
    echo ""
...
clawdock-help() {
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}./docs${_CLR_RESET}"
  echo ""
}
