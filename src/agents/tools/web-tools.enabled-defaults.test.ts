...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
            ...
            search_results: [
              { title: "Longxia", url: "https://longxia.local/docs", content: "docs" },
            ],
          }),
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://longxia.local/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://longxia.local/docs"]);
