...
const OPENCLAW_REPO_URL =
  process.env.OPENCLAW_GIT_REPO_URL?.trim() || "https://github.com/Jakecoin/longxia.git";
...
export async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or a Longxia checkout.`,
      );
    }
