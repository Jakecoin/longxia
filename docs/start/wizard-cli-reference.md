...
- Channels and providers (Google Chat, Signal, Slack, BlueBubbles, and bundled providers still enabled in this fork)
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - [Slack](/channels/slack): workspace credentials
    - [Signal](/channels/signal): optional `signal-cli` install + account config
    - [BlueBubbles](/channels/bluebubbles): recommended for iMessage; server URL + password + webhook
    - [iMessage](/channels/imessage): legacy `imsg` CLI path + DB access
    - Telegram, Discord, WhatsApp, and plugin-installed channels are disabled in Longxia
...
- `channels.signal.*`, `channels.imessage.*`, plus any still-enabled built-in channel credentials
- Channel allowlists for the channels still enabled in this fork when you opt in during prompts
...
Channel credentials are stored under `~/.openclaw/credentials/<channel>/<accountId>/` when a channel in this fork requires local credential state.
