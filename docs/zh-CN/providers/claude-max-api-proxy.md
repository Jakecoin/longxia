...
Longxia 不提供这个社区包的公共镜像，也不会在默认流程里自动安装它。如果你确实要用，应当通过你自己的受控源引入，并按内部源策略完成安装。
...
- 这是外部社区依赖，不属于 Longxia 默认内置能力
- Longxia 不提供公开镜像、安装入口或捆绑副本
- 如果你的团队必须使用，请固定到内部审查过的源，并单独维护说明
