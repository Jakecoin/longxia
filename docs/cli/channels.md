...
read_when:
  - You want to add/remove channel accounts for the built-in Longxia channels
...
openclaw channels capabilities --channel slack --target "#ops"
...
```bash
openclaw channels add --channel slack --token <bot-token>
openclaw channels remove --channel slack --delete
```
...
```bash
openclaw channels login --channel <supported-channel>
openclaw channels logout --channel <supported-channel>
```
...
- `--target` accepts `channel:<id>` or a provider-specific target identifier when the selected channel supports it.
- Probes are provider-specific and only run for the channels enabled in this Longxia build. Channels without probes report `Probe: unavailable`.
