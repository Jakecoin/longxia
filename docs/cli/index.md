...
Inspect bundled extension state in Longxia:

- `openclaw plugins list` — discover plugins (use `--json` for machine output).
- `openclaw plugins info <id>` — show details for a plugin.
- All mutating plugin commands are blocked in this fork (`install`, `enable`, `disable`, `update`, `uninstall`, `doctor`).

Longxia only supports preapproved capabilities bundled into the deployment. See [/plugin](/tools/plugin).
...
Manage chat channel accounts for the built-in channels that remain enabled in Longxia.
...
- `channels login`: interactive channel login (only for channels that still expose a login flow in this fork).
- `channels logout`: log out of a channel session (if supported).

Common options:

- `--channel <name>`: use one of the channels exposed by `openclaw channels capabilities`
...
- `gateway install` defaults to Node runtime; bun is **not recommended** in this fork.
